...
function get_required_value(parsed_env, key) {
  const value = parsed_env[key]?.trim();

  if (!value) {
    throw new Error(`Missing ${key} in ${path.basename(env_file_path)}`);
  }
...
if (!fs.existsSync(env_file_path)) {
  console.error(`Missing env file: ${env_file_path}`);
  process.exit(1);
}

const parsed_env = parse_env_file(fs.readFileSync(env_file_path, "utf8"));
